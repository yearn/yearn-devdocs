...
- Vaults can be either [multi-strategy](/developers/v3/overview#definitions) or [single-strategy (Tokenized Strategy)](/developers/v3/overview#definitions).
- Multi-strategy vaults tokens are prefixed with `yv` and appended with a ["category"](/developers/v3/Integrating_v3#category), so a v3 vault for USDC with a category type of `1` gives the user `yvUSDC-1`.
