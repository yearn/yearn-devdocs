...
The most important implication is that the `withdraw` and `redeem` functions as presented in ERC4626, the liquidity to redeem shares will be the one in the vault. No strategies will be passed to the redeem function to withdraw with the `withdraw` and `redeem` function. 
