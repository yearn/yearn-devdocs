...
- Market Caps: Calculating market caps multiple times a day and alerting if the market cap is too high and should be lowered. We don't immediately lower the market cap in Morpho contracts, but we lower it in internal reallocation configuration. This allows us to keep the vault's allocation below calculated caps and set higher caps if the market gets healthier without waiting for the 3-day timelock delay. If the caps keep being too high over a long period of time, we will lower the cap in the Morpho contracts to ensure maximum safety.
- Collateral Liquidity: Calculating multiple times a day to verify that collaterals backing risky borrowing positions have enough on-chain liquidity to get liquidated for the borrowed asset without causing high slippage and becoming unprofitable to liquidate, which could cause bad debt in lending vaults.
