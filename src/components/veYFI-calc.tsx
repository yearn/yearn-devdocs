...
const VeYFICalculator: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setGaugeData(Array.isArray(resolvedGaugeData) ? resolvedGaugeData : [])
    }
    fetchData()
  }, [publicClient])
  ...
  const BoostChart = ({ data, xVar, dataKey }) => {
          ...
          >
            ...
            <XAxis
              ...
              tickFormatter={(tick) => {
                ...
                if (maxX <= 5) {
                  return tick.toFixed(2) // Show small numbers with 2 decimal places
                } else if (maxX <= 10) {
